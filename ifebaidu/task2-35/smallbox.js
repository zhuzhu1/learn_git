...
function init()
{
    ...
    var refreshObj = document.getElementById("refresh");
    refreshObj.onclick = function(){
        refresh(div1Obj, div2Obj);
    }

    var exeType = ["TRA LEF", "TRA TOP", "TRA RIG", "TRA BOT", "MOV LEF", "MOV TOP", "MOV RIG", "MOV BOT", "GO",];
    var executeObj = document.getElementById("execute");
    executeObj.onclick = function(){
        var inputareaObj = document.getElementById("inputarea");
        debugger;
        var inputStr = inputareaObj.value;
        var inputArr = inputStr.split("\n");
        for(var i = 0; i < inputArr.length; i++)
        {
            if(exeType.indexof(inputArr[i]) != -1)
            {
...
function refresh(obj, obj2)
{
    obj.style.top = "165px";
    obj.style.left = "165px";
    obj2.className = "div2up";
}
